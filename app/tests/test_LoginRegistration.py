...
# client - the client to do requests with
    def setUp(self):
        user1active=User.objects.create_user('test@test.com',password='123456')
        user1active.save()
        self._user1active=user1active

        user2inactive=User.objects.create_user('test2@test.com',password='none')
        user2inactive.is_active=False
        user2inactive.save()
        self._user2inactive=user2inactive
        self._client=Client()

## Tests if on incorrect login the client received a failure message and is directed to the login page
    def test_loginFails(self):
        #Testing that a user is not logged in with an incorrect password
        response=self._client.post('/login/',{'email':self._user1active.username,'password':'wrong'})
    ...
    def test_loginSuccess(self):
        ...
        #Testing that a user is directed tothe "Startseite" after an correct password
