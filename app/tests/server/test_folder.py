...
class FolderTestClass(TestCase):
    ...
    def setUp(self):
        ...
        self._user1_binary1_size = user1_binfile1.write(
    ...
    def test_createDir(self):
        ...
        # antwort sollte id, name, parentfolderid und parentfoldername
        # enthalten
        ...
        # Teste, ob der Ordner wirklich existiert und mit dem Werten, die
        # zurückgegeben wurden, übereinstimmt
        ...
        response = util.documentPoster(
            self, command='createdir', idpara=folder1.id, name='root')
        dictionary = util.jsonDecoder(response.content)
        self.assertTrue(
            Folder.objects.filter(id=dictionary['response']['id']).exists())
        # Teste, ob ein Verzeichnis zwei gleichnamige Unterverzeichnisse haben
        # kann
        response = util.documentPoster(
            self, command='createdir', idpara=folder1.id, name='root')
        dictionary = util.jsonDecoder(response.content)
        self.assertEqual(
            dictionary['response'], ERROR_MESSAGES['FOLDERNAMEEXISTS'])

        # Teste, wie es sich mit falschen Angaben verhält

        # Teste ob user1 in einem Project vom user2 ein Verzeichnis erstellen
        # kann
        response = util.documentPoster(
            self, command='createdir', idpara=self._user2_project1.id, name='IDONOTEXISTDIR')
        dictionary = util.jsonDecoder(response.content)
        ...
        response = util.documentPoster(
            self, command='createdir', idpara=self._user1_project1.id, name=' ')
        dictionary = util.jsonDecoder(response.content)
        serveranswer = dictionary['response']
        self.assertEqual(serveranswer, ERROR_MESSAGES['BLANKNAME'])

    # Teste das Löschen eines Ordners
    # Teste, ob Unterordner und zum Ordner gehörende Dateien auch gelöscht
    # werden
    def test_rmDir(self):
        self.client.login(
            username=self._user1.username, password=self._user1._unhashedpw)
        ...
        # Stelle sicher, dass zu diesem Zeitpunkt der rootordner von project1
        # noch existiert
        self.assertTrue(Folder.objects.filter(id=oldrootfolderid).exists())
        # Stelle sicher, dass es zu diesem Projekt mind. ein Unterverzeichnis
        # existiert + mind. eine Datei
        self.assertEqual(
            self._user1_project1_folder1.parent, self._user1_project1.rootFolder)
        self.assertTrue(File.objects.filter(id=oldfileid).exists())

        # Teste, dass man keinen Rootfolder löschen kann
        response = util.documentPoster(
            self, command='rmdir', idpara=self._user1_project1_root.id)
        dictionary = util.jsonDecoder(response.content)
        serveranswer = dictionary['response']
        self.assertEqual(dictionary['status'], FAILURE)
        ...
        response = util.documentPoster(
            self, command='rmdir', idpara=self._user1_project1_folder2.id)

        self.assertFalse(
            Folder.objects.filter(id=oldtodeletefolder.id).exists())

        # Unterverzeichnisse und Dateien sollten dabei auch gelöscht werden!
        self.assertFalse(
            Folder.objects.filter(id=oldsubdeletefolder.id).exists())
        ...
        response = util.documentPoster(
            self, command='rmdir', idpara=self._user2_project1_folder1.id)
    ...
    def test_renameDir(self):
        ...
        # Teste auf richtige Response Daten bei einer Anfrage, die
        ...
        response = util.documentPoster(
            self, command='renamedir', idpara=self._user1_project1_folder2.id, name='newname')
        dictionary = util.jsonDecoder(response.content)
        self.assertEqual(
            dictionary['response'], ERROR_MESSAGES['FOLDERNAMEEXISTS'])
        ...
        response = util.documentPoster(
            self, command='renamedir', idpara=self._user2_project1.id, name='ROFL')
        dictionary = util.jsonDecoder(response.content)
        serveranswer = dictionary['response']
        ...
        response = util.documentPoster(
            self, command='renamedir', idpara=self._user1_project1_folder2.id, name='?/')
        util.validateJsonFailureResponse(
    ...
    def test_moveDir(self):
        ...
        dictionary = util.jsonDecoder(response.content)
        # Teste, ob beim richtiger Anfrage eine Erfolgsmeldung zurückgegeben
        # wird
        self.assertEqual(dictionary['status'], SUCCESS)

        # Teste, ob ob man zwischen Projekten Dateuen verschieben darf (sollte
        # man dürfen)
        ...
        response = util.documentPoster(
            self, command='movedir', idpara=-1, idpara2=self._user1_project1_folder2.id)
        self.assertEqual(util.jsonDecoder(response.content)['status'], FAILURE)

        # Teste, ob Fehlermeldung, falls ein Ordner verschoben wird, auf dem
        # der User keine Rechte hat
        response = util.documentPoster(
            self, command='movedir', idpara=self._user2_project1_folder1.id, idpara2=self._user1_project1_folder1.id)
        util.validateJsonFailureResponse(
            self, response.content, ERROR_MESSAGES['NOTENOUGHRIGHTS'])

        # Teste, ob Fehlermeldung, falls ein Ordner in einen Ordner verschoben
        # wird, auf die der User keine Rechte hat
        response = util.documentPoster(
            self, command='movedir', idpara=self._user1_project1_folder1.id, idpara2=self._user2_project1_folder1.id)
        util.validateJsonFailureResponse(
            self, response.content, ERROR_MESSAGES['NOTENOUGHRIGHTS'])

    # Teste, dass beim Verschieben eines Ordners nach überprüft wird, ob es
    # einen gleichnamigen Unterordner im neuen Parentordner schon gibt
    def test_moveDir2(self):
        ...
        self._user1_project1_folder2_subfolder1.save()
        # Versuche subfolder in den gleichen Ordner wie parentfolder zu
        # verschieben
        response = util.documentPoster(
            self, command='movedir', idpara=self._user1_project1_folder2_subfolder1.id, idpara2=self._user1_project1_folder2.id)
        dictionary = util.jsonDecoder(response.content)
        self.assertEqual(
            dictionary['response'], ERROR_MESSAGES['FOLDERNAMEEXISTS'])
    ...
    def test_listfiles(self):
        # Anfrage der Struktur von _user1_project1_folder2 (Aufbau siehe SetUp
        # Methode)
        response = util.documentPoster(
            self, command='listfiles', idpara=self._user1_project1_folder2.id)
        ...
        response = util.documentPoster(
            self, command='listfiles', idpara=self._user2_project1_root.id)
        ...
        response = util.documentPoster(
            self, command='listfiles', idpara=self._user1_invalidid)
