...
class AuthLoginTestClass(TestCase):
    ...
    def test_loginFailInactiveUser(self):
        ...
        self.assertContains(
            response, ERROR_MESSAGES['INACTIVEACCOUNT'].format(self._user2.username))
    ...
    # Test that if an already logged in user tries to login, a redirect to the
    # start page will be done
...
class AuthRegistrationTestClass(TestCase):
    ...
    def test_registrationFailFirstNameIllegalChar(self):
        ...
        self.assertContains(
            response, ERROR_MESSAGES['INVALIDCHARACTERINFIRSTNAME'])

    # Test that you will be directed to the startseite when trying to register
    ...
    def test_registrationRedirectWhenLoggedIn(self):
        new_user = User.objects.create_user(username=self._user1_email,
                                            ...
                                            first_name=self._user1_first_name)
        self._client.login(
            username=self._user1_email, password=self._user1_password1)
        self.assertIn('_auth_user_id', self._client.session)
