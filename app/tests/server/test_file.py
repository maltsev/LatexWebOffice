...
class FileTestClass(TestCase):
    ...
    def setUp(self):
        ...
        self._user1_project1 = Project.objects.create(
        ...
        self._user2_project1 = Project.objects.create(
        ...
        # Erstelle eine .tex Datei für user1 in user1_project1_root (Projekt
        # root Verzeichnis)
        ...
        self._user1_tex2 = TexFile(
            name='tex2.tex', folder=self._user1_project1_root, source_code='user1_tex2\n')
        self._user1_tex2.save()
        self._user1_tex3 = TexFile(
            name='TEX2.tex', folder=self._user1_project1_folder1, source_code='user1_tex3\n')
        self._user1_tex3.save()

        self._user1_dir = os.path.join(
            settings.FILEDATA_URL, str(self._user1.id))
        self._user1_project1_dir = os.path.join(
            self._user1_dir, str(self._user1_project1.id))
        if not os.path.isdir(self._user1_project1_dir):
            os.makedirs(self._user1_project1_dir)

        # Erstelle eine Binärdatei für user1 in
        # user1_project1_folder2_subfolder1
        self._user1_binary1 = BinaryFile(
            name='test.bin', folder=self._user1_project1_folder2_subfolder1)
        self._user1_binary1.save()
        self._user1_binary1_str = 'user1_binary1'
        self._file_path1 = os.path.join(
            self._user1_project1_dir, str(self._user1_binary1.id))
        self._user1_binfile1 = open(self._file_path1, 'w')
        self._user1_binary1_size = self._user1_binfile1.write(
            self._user1_binary1_str)
        self._user1_binfile1.close()

        # Erstelle eine TexDatei für user1 in user1_project1_folder2_subfolder2
        self._user1_binary2 = BinaryFile(
            name='test2.tex', folder=self._user1_project1_folder2_subfolder1)
        self._user1_binary2.save()
        self._user1_binary2_str = 'user1_binary2'
        self._file_path2 = os.path.join(
            self._user1_project1_dir, str(self._user1_binary2.name))
        self._user1_binfile2 = open(self._file_path2, 'w')
        self._user1_binary2_size = self._user1_binfile2.write(
            self._user1_binary2_str)
        self._user1_binfile2.close()

        # Erstelle eine BildDatei für user1 in
        # user1_project1_folder2_subfolder2
        self._user1_binary3 = BinaryFile(
            name='test2.jpg', folder=self._user1_project1_folder2_subfolder1)
        self._user1_binary3.save()
        self._user1_binary3_str = 'user1_binary3'
        self._file_path3 = os.path.join(
            self._user1_project1_dir, str(self._user1_binary3.name))
        self._user1_binfile3 = open(self._file_path3, 'w')
        self._user1_binary3_size = self._user1_binfile3.write(
            self._user1_binary3_str)
    ...
    def test_createtexfile(self):
        ...
        # Sende Anfrage zum erstellen einer neuen .tex Datei mit einem Namen,
        # der bereits im selben Ordner existiert
        ...
        util.validateJsonFailureResponse(
            self, response.content, ERROR_MESSAGES['FILENAMEEXISTS'])

        # Sende Anfrage zum erstellen der Datei als user1 mit der folderid die
        # user2 gehört
        ...
        # Sende Anfrage zum erstellen der Datei als user1 mit einer folderid
        # die nicht existiert
        response = util.documentPoster(
            self, command='createtex', idpara=100000, name='newname')

        # überprüfe die Antwort des Servers
        # sollte failure als status liefern
        # sollte die Fehlermeldung ERROR_MESSAGES['DIRECTORYNOTEXIST'] liefern
        util.validateJsonFailureResponse(
            self, response.content, ERROR_MESSAGES['DIRECTORYNOTEXIST'])

        # Sende Anfrage zum erstellen der Datei als user1 mit einem ungültigen
        # Namen
        response = util.documentPoster(self, command='createtex', idpara=self._user1_project1_folder1.id,
                                       name='file1<>.tex')
    ...
    def test_updatefile(self):
        ...
        # die in der Datenbank gespeicherte .tex Datei sollte als source_code
        # nun den neuen Inhalt besitzen
        self.assertEqual(TexFile.objects.get(
            id=self._user1_tex1.id).source_code, self._user1_tex1_newcode)
        ...
        util.validateJsonFailureResponse(
            self, response.content, ERROR_MESSAGES['NOPLAINTEXTFILE'])

        # Sende Anfrage zum ändern der Datei als user1 mit der fileid einer
        # .tex Datei die user2 gehört
        ...
        util.validateJsonFailureResponse(
            self, response.content, ERROR_MESSAGES['FILENOTEXIST'])

        # Sende Anfrage zum ändern der Datei als user1 mit einer fileid.
        # Die Datei soll nun nur noch einen leeren String beinhalten
        response = util.documentPoster(
            self, command='updatefile', idpara=self._user1_tex1.id, content='')
        util.validateJsonSuccessResponse(self, response.content, {})
    ...
    def test_deletefile(self):
        ...
        response = util.documentPoster(
            self, command='deletefile', idpara=self._user1_tex1.id)
        ...
        self.assertFalse(
            PlainTextFile.objects.filter(id=self._user1_tex1.id).exists())

        # Sende Anfrage zum Löschen der Binärdatei
        response = util.documentPoster(
            self, command='deletefile', idpara=self._user1_binary1.id)
        ...
        # in der Datenbank sollte die Datei nun nicht mehr vorhanden sein
        self.assertFalse(
            BinaryFile.objects.filter(id=self._user1_binary1.id).exists())

        # Sende Anfrage zum Löschen einer Datei als user1 mit der fileid einer
        # .tex Datei die user2 gehört
        response = util.documentPoster(
            self, command='deletefile', idpara=self._user2_tex1.id)
        ...
        # die auf dem Server in der Datenbank nicht existiert
        response = util.documentPoster(
            self, command='deletefile', idpara=self._user1_invalidid)
    ...
    def test_renamefile(self):
        ...
        # Sende Anfrage zum Umbennen einer Datei als user1 mit der fileid einer
        # .tex Datei die user2 gehört
    ...
    def test_movefile(self):
        # Sende Anfrage zum Verschieben der .tex Datei in den Unterorder
        # folder1 des Projektes
        ...
        # Sende Anfrage zum Verschieben der Binärdatei in den Unterorder
        # folder1 des Projektes
        ...
        # Sende Anfrage zum Verschieben einer Datei als user1 mit der fileid
        # einer .tex Datei die user2 gehört
        ...
        # Sende Anfrage zum verschieben einer Datei mit einem Namen, der
        # bereits im selben Ziel Ordner existiert
    ...
    def test_uploadfiles(self):
        ...
        # Es sollte eig. immer 'success' ausgegeben werden, da auch 'success'
        # kommen sollte, selbst wenn keine einzige Datei akezeptiert wurde
        self.assertEqual(
            util.jsonDecoder(response.content)['status'], 'success')

        # Es sollten genau 2 Dateien vom Server akzeptiert werden: test2.tex
        # und test3.jpg
        ...
        self.assertEqual(
            serveranswer['failure'][0]['reason'], ERROR_MESSAGES['ILLEGALFILETYPE'])

        # Teste, ob Fehlermeldung, falls versucht wird, Dateien in einen Ordner
        # hochzuladen, auf dem der User keine Rchte hat
        dic['id'] = self._user2_project1_folder1.id
        response = self.client.post('/documents/', dic)
        util.validateJsonFailureResponse(
            self, response.content, ERROR_MESSAGES['NOTENOUGHRIGHTS'])

        # Falls keine Dateien versendet wurden, sollte ebenfalls eine
        # Fehlermeldung zurückgegeben werden
        dic['files'] = None
        dic['id'] = self._user1_project1_root.id
        response = self.client.post('/documents/', dic)
        util.validateJsonFailureResponse(
            self, response.content, ERROR_MESSAGES['NOTALLPOSTPARAMETERS'])

    # Teste download von Dateien auf dem Server zum Client
    def test_downloadfile(self):
        # Sende Anfrage zum Downloaden der test.bin Datei
        response = util.documentPoster(
            self, command='downloadfile', idpara=self._user1_binary1.id)

        # überprüfe die Antwort des Servers
        # der Content-Type sollte 'application/octet-stream' sein
        self.assertEqual(response['Content-Type'], 'application/octet-stream')
        # Content-Length sollte 'self._user1_binary1_size' sein (Dateigröße in
        # bytes)
        self.assertEqual(
            response['Content-Length'], str(self._user1_binary1_size))
        # Content-Disposition sollte 'attachment; filename=b'test.bin'' sein
        self.assertEqual(
            response['Content-Disposition'], 'attachment; filename=b\'test.bin\'')
        # der Inhalt der heruntergeladenen Datei und der Datei auf dem Server
        ...
        response = util.documentPoster(
            self, command='downloadfile', idpara=self._user1_tex1.id)

        # überprüfe die Antwort des Servers
        # der Inhalt der heruntergeladenen Datei und der Datei auf dem Server
        # sollte übereinstimmen
        self.assertEqual(
            self._user1_tex1.source_code, smart_str(response.content))
        ...
        # Content-Disposition sollte 'attachment; filename=b'test.bin'' sein
        self.assertEqual(
            response['Content-Disposition'], 'attachment; filename=b\'main.tex\'')

        # Sende Anfrage zum Download einer Datei als user1 mit der fileid einer
        # .tex Datei die user2 gehört
        response = util.documentPoster(
            self, command='downloadfile', idpara=self._user2_tex1.id)
        ...
        # die auf dem Server in der Datenbank nicht existiert
        response = util.documentPoster(
            self, command='downloadfile', idpara=self._user1_invalidid)
    ...
    def test_fileInfo(self):
        ...
        response = util.documentPoster(
            self, command='fileinfo', idpara=self._user1_binary1.id)
        ...
        # Die Datei, über die Informationen angefordert wurde
        fileobj = self._user1_binary1
        folderobj = self._user1_binary1.folder  # der Ordner, wo fileobj liegt

        # Die zurückgegebenen Informationen sollten mit fileobj und folderobj
        # übereinstimmen
    ...
    def test_latexCompile(self):
        # schicke POST request an den Server mit dem compile Befehl und dem
        # zugehörigen Parameter id:fileid
        response = util.documentPoster(
            self, command='compile', idpara=self._user1_tex1.id)
        ...
        response = util.documentPoster(
            self, command='compile', idpara=self._user1_tex2.id)
        util.validateJsonFailureResponse(
            self, response.content, ERROR_MESSAGES['COMPILATIONERROR'])
